...
import { X, Calendar, Clock, MessageSquare, User, Paperclip, Edit3 } from "lucide-react";
          ...
          >
            ...
            <div className="p-6 overflow-y-auto space-y-5 text-gray-800 dark:text-gray-200">
              ...
              <section className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex items-center gap-2"><Calendar size={16} /> Created: {dayjs(task.createdAt).format("YYYY-MM-DD")}</div>
                <div className="flex items-center gap-2"><Calendar size={16} /> End: {task.endDate || "-"}</div>
                <div className="flex items-center gap-2"><Clock size={16} /> Start Time: {task.startTime || "-"}</div>
                <div className="flex items-center gap-2"><Clock size={16} /> End Time: {task.endTime || "-"}</div>
              </section>
              ...
              <section>
                ...
                <div className="flex flex-wrap gap-2">
                  {task.assigneeTo?.length ? task.assigneeTo.map((email, idx) => (
                    <span key={idx} className="px-3 py-1 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center gap-2 text-sm">
                      <User size={14} /> {email}
                    </span>
              ...
              {/* Comments */}
              <section>
                <h3 className="font-semibold mb-2 flex items-center gap-2"><MessageSquare size={16} /> Comments</h3>
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {comments.map((c) => (
                    <div key={c.id} className="bg-gray-100 dark:bg-gray-800 rounded-lg p-3">
                      <p className="text-sm">{c.text}</p>
                      <p className="text-xs text-gray-500 mt-1">— {c.author}, {c.time}</p>
                    </div>
                  ))}
                </div>
                <div className="flex items-center gap-2 mt-3">
                  ...
                  />
                  <button onClick={handleAddComment} className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Post</button>
                </div>
              </section>
            </div>
            ...
            <div className="flex justify-between items-center p-4 border-t dark:border-gray-700 bg-white dark:bg-gray-900 sticky bottom-0">
              <button onClick={() => onEdit(task)} className="px-4 py-2 rounded bg-blue-500 text-white hover:bg-blue-600 flex items-center gap-2">
                <Edit3 size={16} /> Edit Task
              </button>
              <div className="flex gap-2">
                <button onClick={markPending} className="px-4 py-2 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600">Pending</button>
                <button onClick={markInProgress} className="px-4 py-2 rounded bg-yellow-500 text-white hover:bg-yellow-600">In Progress</button>
                <button onClick={markCompleted} className="px-4 py-2 rounded bg-green-500 text-white hover:bg-green-600">Complete</button>
              </div>
            </div>
          </motion.div>
        </motion.div>
